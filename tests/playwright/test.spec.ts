// [10:29, 12/28/2025] +91 93848 19239: Classroom
// https://www.leafground.com/window.xhtml
// -> click on Open button
// -> switch to child page
// -> Child Page -> Enter Message
// -> close childpage
// -> click on open multiple
// [10:29, 12/28/2025] +91 93848 19239: import test from '@playwright/test'

// import {test} from "@playwright/test"

// test("Multiple Window",async({page,context})=>{


// await page.goto("https://www.leafground.com/window.xhtml")


// //create promise
// const [allwindow]=await Promise.all([
// //action
// context.waitForEvent('page'),
//  page.locator("//span[text()='Open Multiple']").click()
// ])


// //[allwindow]-> [p,ch1,ch2.....]
// const count=allwindow.context().pages()
// console.log(count.length)
// await allwindow.waitForTimeout(4000)


// //Title -> Best way to handle multiple window 
// //DashBroad,WebTable -> child page name
// let dashBroad:any
// let webTable:any


// for(let i=0;i<count.length;i++){
//     const title=await count[i].title()
//     // console.log(title) ->Web Table ,Dashboard
//     // await page.waitForTimeout(4000)


//     if(title==="Dashboard"){
//         //resolve the promise
//         dashBroad=count[i]
//     }else if(title==="Web Table"){
//         webTable=count[i]
//     }
// }
// dashBroad.locator("//input[@id='email']").fill("dilip@testleaf.com")
// dashBroad.waitForTimeout(5000)




// })


// https://www.leafground.com/window.xhtml
// -> click on Open button
// -> switch to child page
// -> Child Page -> Enter Message
// -> close childpage
// -> click on open multiple


import test from "@playwright/test"

test("Handle windows", async({page,context})=> 

    {

        await page.goto("https://www.leafground.com/window.xhtml")



        // page.waitForEvent
        // page.locator("//span[text()='Open']").click()

        const [windows] = await  Promise.all([
        
            context.waitForEvent('page'),
    

         page.locator("//span[text()='Open']").click()


        ])

        const count = windows.context().pages()

        console.log(count.length)
        await windows.waitForTimeout(4000)

        for(let i=0; i<count.length;i++)
        {
     const title=await count[i].title()
     console.log(title) 
    // ->Web Table ,Dashboard
    await page.waitForTimeout(4000) 
           }




    })